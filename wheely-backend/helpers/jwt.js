  ...
  return jwt({
    ...
    isRevoked: async (req, token) => {
      try {
        ...
        const carUserReqInfo = req.path.match(
          `${api}cars/fleets/([0-9a-fA-F]{24})`
        );

        if (bookingReqInfo) bookingID = bookingReqInfo[1];
        if (userReqInfo) userID = userReqInfo[1];
        if (carUserReqInfo) carUserID = carUserReqInfo[1];
        ...
        const allowedOwnerRoutes = [
          ...
          { method: "GET", path: `${api}cars/:carUserID` },
        ...
        if (userRole === "owner") {
          ...
          if (isAllowed && req.method === "GET" && carUserID) {
            const car = await Car.find({ owner: carUserID }).populate("owner");
            if (
              !car ||
              car.length === 0
            ) {
              console.log("Car not found or user is not the owner");
              return true;
            }
          }
